...
void clusterrec(const std::vector<std::vector<float>> &points, int index, std::vector<int> &cluster, std::vector<bool> &processed, KdTree *tree, float distanceTol)
{
	processed[index] = true;
	cluster.push_back(index);
	auto nearbyP = tree->search(points[index], distanceTol);
	for(int nearbyIndex : nearbyP)
	{
		if(processed[nearbyIndex] == false)
		{
			clusterrec(points, nearbyIndex, cluster, processed, tree, distanceTol);
		}
	}
}
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>> &points, KdTree *tree, float distanceTol)
{
	...
	for (int i = 0; i < points.size(); i++)
	{
		if (processed[i] == false)
		{
			...
			clusterrec(points, i, cluster, processed, tree, distanceTol);
